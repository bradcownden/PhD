...
%	\;
	...
	\end{subfigure}
	\caption[Evolution of the spectrum of a threshold temperature solution that has been padded with $100$ modes]{Evolution of a padded threshold temperature solution.}
...
As we have seen, some QP solutions retain a power law spectrum under evolution, but scalar curvature and/or large residuals in the Einstein equations indicate that such data may not persist over timescales $t \sim \epsilon^{-2}$ under fully nonlinear evolution. As a final example, we consider a solution that has been repeatedly perturbed using the method described in \S~\ref{ssec: highT} to a temperature of $T = 20$ without projecting back to the QP plane \emph{at any point}. This solution is then used as seed values to solve \eqref{ttf eqn}. For values of $\jm \lesssim 100$, solutions

It has been suggested that high-temperature QP solutions should exist in a continuous region of temperature space from $T_{min} = d$ to $T_{max} = 2\jm + d$ and that perturbing a low-temperature QP solution to a temperature above $T_{th}$ before implementing regular projections allows for all such solutions to be found. However, we have found that the majority of high temperature solutions are not robust as $\jm \to \infty$ and therefore do no constitute true solutions. 
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{Comparative evolution of modes ${j=0, 25, 50, 75, 99}$ (purple, green, blue, orange).}
	\end{subfigure}
	\;
	\begin{subfigure}[t]{0.45\textwidth}
