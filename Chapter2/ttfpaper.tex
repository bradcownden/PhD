...
Practically speaking, finding solutions to the $j_{max}$ equations that arise from \eqref{ttf eqn} requires truncating the series at a finite value $j_{max} < \infty$. However, a solution must continue to be present and unaffected by increasing $j_{max}$ in order for it to represent a valid solution to the TTF theory. Let us first examine how robust low temperature QP solutions are to the choice of truncation. 
