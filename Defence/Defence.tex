...
\frame[shrink]
{
  ...
  \its<2->{All normalizable: restrictions on indices {\bf and} mass value \uncover<3->{$\to$ \alert<3>{two channels vanish numerically}}}
  \its<4->{One \alert<4>{non-vanishing} channel \uncover<5>{$\to$ evaluate numerically}}
  \ei
