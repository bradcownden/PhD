...
\frame
{
  ...
  \column{0.5\textwidth}
	...
	\only<3>{
	\vspace{-0.6in}
	\includegraphics[scale=0.7]{NNsum4m0}
	}
	}
	\end{figure}
  \end{columns}
  ...
  \only<4>{
	...
	\frac{2 \omega_\ell a_\ell}{\epsilon^2} \frac{db_\ell}{dt} &= - S_\ell a_\ell \bar{A}^2_{\bar \omega}
	\end{align*}
  }
  \end{overlayarea}
}
...
\frame
{
  \frametitle{Results}
  \bi
  \its Confirm two of three resonant channels vanish for massive scalar (all normalizable)\footnotemark
  \its First TTF formulation with time-dependent boundary conditions
  \its No naturally-vanishing source terms for $\omega_\alpha$ considered $\to$ sum resonant channels
  \its Flow equations are linear in amplitude/phase variables $a_\ell(t)$, $b_\ell(t)$
  \its {\bf Further work}: quasi-periodic solutions? Conserved quantities? Energy cascades? Relation to driven holographic CFTs\footnotemark?
  \ei
  
  \footnotetext[12]{{\scr Biasi {\it et al.} [1810.04753]}}
  \footnotetext[13]{{\scr Rangamani {\it et al.} [1502.05726]}}
}
...
\frame[noframenumbering]
{
   ...
   \its PhD Committee: (),  (), (),  ()
   \its Co-authors: Nils Deppe (Cornell)
...
\frame[noframenumbering]
{
  ...
  \its M. Rangamani, M. Rozali and A. Wong, \emph{Driven Holographic CFTs}, JHEP 04 (2015) 093, [1502.05726].
  \ei
}
}
